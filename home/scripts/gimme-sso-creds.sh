...
AWS_PROFILE="${2:-$(aws configure list-profiles | grep -v default | fzf --tmux --reverse --prompt='AWS Profile> ')}"
#
# if no current aws sso session, then authenticate
# if ! aws sts get-caller-identity --profile "${AWS_PROFILE}" &>/dev/null; then
#   aws sso login --profile "${AWS_PROFILE}" &>/dev/null
# fi
# I want to see the output of this...
...
$([ "${AWS_SESSION_TOKEN}" != "null" ] && echo "aws_session_token = ${AWS_SESSION_TOKEN}")

EOF

  echo "Success! Credentials written to ${HOME}/.aws/credentials"
